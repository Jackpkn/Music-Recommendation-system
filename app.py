...
#
df = pd.read_csv("/home/jackpk/Downloads/Emotion-based-music-recommendation-system/muse_v3.csv")
...
def fun(emotion_list):
    data = pd.DataFrame()
    total_songs = 30  # Total number of songs to recommend

    if not emotion_list:
        return data

    # Define a dictionary to map emotions to dataframes
    emotion_to_df = {
        'Neutral': df_neutral,
        'Angry': df_angry,
        'fear': df_fear,
        'happy': df_happy,
        'Sad': df_sad  # Assuming 'Sad' for the 'else' case
    }

    # Count occurrences of each emotion
    emotion_counts = Counter(emotion_list)

    # Calculate songs per emotion, ensuring at least 1 song per detected emotion
    base_songs_per_emotion = max(1, total_songs // len(emotion_counts))
    remaining_songs = total_songs - (base_songs_per_emotion * len(emotion_counts))

    # Distribute remaining songs based on emotion frequency
    for emotion, count in emotion_counts.most_common():
        n_songs = base_songs_per_emotion
        if remaining_songs > 0:
            additional = min(count, remaining_songs)
            n_songs += additional
            remaining_songs -= additional

        df = emotion_to_df.get(emotion, df_sad)  # Default to df_sad if emotion not found
        data = pd.concat([data, df.sample(n=n_songs)], ignore_index=True)
...
model.load_weights(
    '/home/jackpk/Downloads/Emotion-based-music-recommendation-system/model.h5')
...
face = cv2.CascadeClassifier(
    '/home/jackpk/Downloads/Emotion-based-music-recommendation-system/haarcascade_frontalface_default.xml')
if face.empty():
